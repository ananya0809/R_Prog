...
typeof(x)
x[1]
# the result of x[1] is the first value in the container c() -> 1. As R counts from 1, the value of x[1] is the very first element in the container.
```
...
# FILL ME IN
sum(x==1)
# in the above code chunk as there were 2 True values for x==1, sum of them is displayed in this code chunk.
```
...
# FILL ME IN
x[x==1]
# here it returns the values in x where the indices are True for the relational statement.
```
...
```{r}
# FILL ME IN
read.table("simple.txt",header = TRUE,dec = '.',stringsAsFactors = FALSE)
simple<-read.table("simple.txt",header = TRUE,dec = '.',stringsAsFactors = FALSE)
ncol(simple)
colnames(simple)
```
...
```{r}
# FILL ME IN
simple<-read.table("simple.txt",header = FALSE,dec = '.',stringsAsFactors = FALSE, skip=1)
colnames(simple)
...
```{r}
# FILL ME IN
read.table("simple.txt",header = TRUE,dec = '.',stringsAsFactors = FALSE,nrows=4)
...
```{r}
# FILL ME IN
read.csv("planets_small.csv",colClasses = c("character", rep("factor", 3)))
planets<-read.csv("planets_small.csv",colClasses = c("character", rep("factor", 3)))
summary(planets)
```
...
```{r}
# FILL ME IN
emline<-read.csv("emline.csv")
sfrCol<-unlist(emline['sfr'])
print("mean and median for orginal values")
mean(sfrCol)
median(sfrCol)
print("mean and median for updated values")
sfrCol[sfrCol == -9999.0000] <- NA
mean(sfrCol, na.rm=TRUE)
median(sfrCol, na.rm=TRUE)
```
